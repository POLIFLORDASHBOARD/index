    ...
    <div>
      ...
      {vista==="calendario"&&(()=>{
        ...
        <div style={{display:"flex",flexDirection:"column" as const,gap:8}}>
          ...
          {cots.filter((cot:any)=>{const _vend=filtroVendCot==="todos"||(cot.vendedor||vendedorDesdeFolio(cot.folio||""))===filtroVendCot;const _q=busqCot.toLowerCase();const _busq=!busqCot||(cot.cliente_nombre||"").toLowerCase().includes(_q)||(cot.folio||"").toLowerCase().includes(_q)||(cot.lugar_evento||"").toLowerCase().includes(_q)||(cot.cliente_tel||"").includes(busqCot);return (busqCot?_busq:(_vend&&_busq))}).filter((cot:any)=>true
